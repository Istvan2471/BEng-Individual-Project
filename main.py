...
parser.add_argument('--batch-size', type=int, default=250, metavar='B', help='Batch size')
...
parser.add_argument('--value_learning-rate', type=float, default=8e-7, metavar='α', help='Learning rate') 
...
parser.add_argument('--test-interval', type=int, default=2500, metavar='I', help='Test interval (episodes)')
...
parser.add_argument('--paths-to-sample', type=int, default=20, metavar='N', help='Number of paths to sample in BPS')
parser.add_argument('--violation-threshold', type=int, default=3, metavar='ε', help='Violation threshold in BPS')
parser.add_argument('--stickiness', type=float, default=0.1, metavar='ε', help='Violation threshold in BPS')
...
metrics = {'steps': [], 'episodes': [], 'train_rewards': [], 'test_episodes': [], 'test_rewards': [], 
           'observation_loss': [], 'reward_loss': [], 'kl_loss': [], 'actor_loss': [], 'value_loss': [], 'violation_loss': [], 
           'violation_count': []}
...
env = Env(args.env, args.symbolic_env, args.seed, args.max_episode_length, args.action_repeat, args.bit_depth, args.stickiness)
...
if args.models != '' and os.path.exists(args.models):
  ...
  violation_model.load_state_dict(model_dicts['violation_model'])
...
def update_belief_and_act(args, env, planner, transition_model, violation_model, observation_model, encoder, belief, posterior_state, action, observation, violation, shield, explore=False):
  ...
  # if not (args.paths_to_sample == 0) and not (args.paths_to_sample == 999):
  # if episode > 60:# or (episode in range(60, 100) and episode % 2 == 0):
  # shield_action, shield_interfered = shield.step(action)#shield.step(belief, posterior_state, action, observation_model, planner, observation, encoder)
  old_action = action
  ...
  reward -= 5 if violation else 0
    
  return belief, posterior_state, old_action, next_observation, reward, violation, done
  # else:
  #   next_observation, reward, violation, done = env.step(action.cpu() if isinstance(env, EnvBatcher) else action[0].cpu())# action[0].cpu())  # Perform environment step (action repeats handled internally)
  #   reward -= 5 if violation else 0
...
if args.test:
  ...
  encoder.eval()
  shield=GridWorldShield(env)
  # shield = BoundedPrescienceShield(transition_model, violation_model, violation_threshold=args.violation_threshold, paths_to_sample=args.paths_to_sample)
  with torch.no_grad():
    rewards = []
    violations = []
    steps_till_1m = []
    for _ in tqdm(range(args.test_episodes)):
      ...
      for t in pbar:
        belief, posterior_state, action, observation, reward, violation, done = update_belief_and_act(args, env, planner, transition_model, violation_model, observation_model, encoder, belief, posterior_state, action, observation.to(device=args.device), violation, shield)
        total_reward += reward
      ...
      steps_till_1m.append(steps_till)
  print('Average Steps Till 1m From Edge: ', np.mean(steps_till_1m), np.std(steps_till_1m))
...
for episode in tqdm(range(metrics['episodes'][-1] + 1, args.episodes + 1), total=args.episodes, initial=metrics['episodes'][-1] + 1):
  # Model fitting
  losses = []
  model_modules = transition_model.modules+encoder.modules+observation_model.modules+reward_model.modules+violation_model.modules

  print("training loop")
  for s in tqdm(range(args.collect_interval)):
    # Draw sequence chunks {(o_t, a_t, r_t+1, terminal_t+1)} ~ D uniformly at random from the dataset (including terminal flags)
    observations, actions, rewards, violations, nonterminals = D.sample(args.batch_size, args.chunk_size) # Transitions start at time t = 0
    ...
    embedded_observations = bottle(encoder, (observations[1:], ))
    beliefs, prior_states, prior_means, prior_std_devs, posterior_states, posterior_means, posterior_std_devs = transition_model(init_state, actions[:-1], init_belief, embedded_observations, nonterminals[:-1])
    ...
    if args.worldmodel_LogProbLoss:
      ...
      violation_loss = F.cross_entropy(
        bottle(violation_model, (beliefs, posterior_states, )).reshape(len(violations[:-1]) * len(violations) * 25, 2), 
        violations[:-1].reshape(len(violations[:-1]) * len(violations) * 25),
    ...
    if isinstance(env, GridEnv):
      imagined_observations = observation_model.forward(imged_beliefs, imged_prior_states)
      returns = estimate_fm_value(imged_observations, imged_reward)
    else:
      returns = lambda_return(imged_reward, value_pred, bootstrap=value_pred[-1], discount=args.discount, lambda_=args.disclam)
  ...
  lineplot(metrics['episodes'][-len(metrics['violation_loss']):], metrics['violation_loss'], 'violation_loss', results_dir)
  ...
  with torch.no_grad():
    ...
    for t in pbar:
      # print("step",t)
      belief, posterior_state, action, next_observation, reward, violation, done = update_belief_and_act(args, env, planner, transition_model, violation_model, observation_model, encoder, belief, posterior_state, action, observation.to(device=args.device), violation, shield, explore=True)
      D.append(observation, action.cpu(), reward, violation, done)
      total_reward += reward
      observation = next_observation
      if violation:
        violations += 1
    ...
    lineplot([x for x,y in metrics['violation_count']], [y for x,y in metrics['violation_count']], 'violation_count', results_dir)
  ...
  if episode % args.test_interval == 0:
    ...
    shield = ShieldBatcher(BoundedPrescienceShield, test_envs.envs, transition_model, violation_model, violation_threshold=args.violation_threshold, paths_to_sample=args.paths_to_sample)
    with torch.no_grad():
      ...
      violation = torch.zeros(1,1, device=args.device)
      pbar = tqdm(range(args.max_episode_length // args.action_repeat))
      for t in pbar:
        belief, posterior_state, action, next_observation, reward, violation, done = update_belief_and_act(args, test_envs, planner, transition_model, violation_model, observation_model, encoder, belief, posterior_state, action.cuda(), observation.to(device=args.device), violation, shield)
  ...
  print("episodes: {}, total_steps: {}, train_reward: {}, violations: {} ".format(metrics['episodes'][-1], metrics['steps'][-1], metrics['train_rewards'][-1], metrics['violation_count'][-1][1]))
  ...
  if episode % args.checkpoint_interval == 0:
    torch.save({'transition_model': transition_model.state_dict(),
                ...
                'violation_model': violation_model.state_dict(),
